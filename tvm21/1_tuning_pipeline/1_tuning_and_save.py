...
TOTAL_TRIALS = 100  # Change to 20000 for better performance if needed
target = tvm.target.Target("nvidia/jetson-agx-orin-64gb")  # Change to your target device
...
if not IS_IN_CI:
    ...
    ex.export_library('resnet18.so')
    
    from tvm.ir import save_json
    with open('resnet18_relax.json', 'w') as f:
        f.write(save_json(mod))
    
    save_params(params, 'resnet18.bin')